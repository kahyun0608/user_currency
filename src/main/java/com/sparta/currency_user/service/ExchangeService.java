...
public class ExchangeService {
    ...
    public List<ExchangeResponseDto> findExchangeRequest (Long userId) {
        userRepository.findById(userId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "사용자를 찾을 수 없습니다."));
    ...
    public ExchangeResponseDto cancelExchange (Long exchangeId) {
        UserCurrency exchangeFoundById = exchangeRepository.findById(exchangeId).orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND, "해당하는 환전 요청을 찾을 수 없습니다."));
